...
function Traffic() {
  mapboxgl.accessToken =
    "pk.eyJ1IjoidGVtcGVyb3J5IiwiYSI6ImNsYnV2bWtiMzFvcHgzb212c3F0YWJ4ZGoifQ.kVv9N0OjBaZClGM6p1jeVw";
  ...
  const handleClick = async () => {
    serverAPI
      .enterExit(company._id, currentLocation._id, nextState)
      .then((response) => {
        if (nextState == "Enter") {
          setNextState("Exit");
        } else {
  ...
  useEffect(() => {
    ...
    serverAPI
      .lastEnterExitState(companyobj._id)
      .then((lastEnterExitState) => {
        serverAPI
          .companyLocation(companyobj._id)
          .then((companyLocations) => {
            ...
            if (lastEnterExitState.type == "Absent") {
              ...
              if (lastEnterExitState.type == "Enter") {
                setLastStateMessage(
                  `شما ${new Intl.DateTimeFormat("fa-IR", {
                    year: "numeric",
                    month: "long",
                    day: "numeric",
                    hour: "numeric",
                    minute: "numeric",
                  }).format(lastEnterExitState.date)} وارد شدید`
                );
                ...
                setLastStateMessage(
                  `شما ${new Intl.DateTimeFormat("fa-IR", {
                    ...
                    hour: "numeric",
                    minute: "numeric",
                  }).format(lastEnterExitState.date)} خارج شدید`
                );
                enterExitState = "ورود به ";
                setNextState("Enter");
              }
            ...
            if (navigator.geolocation) {
              ...
              navigator.geolocation.watchPosition(function (position) {
                ...
                for (let index = 0; index < companyLocations.length; index++) {
                  ...
                  const cl = {
                    lat: companyLocation.long,
                    lng: companyLocation.lat,
                  };
                  const ul = {
    ...
    <div className="background-container">
      ...
      <div className="traffic-main">
        ...
        <div className="traffic__time--login">
          <div>
            <p className="traffic__text paragraph">{lastStateMessage}</p>
          </div>
        ...
        <button
          ...
          className="traffic__btn-container traffic__btn--size"
          onClick={handleClick}
        >
